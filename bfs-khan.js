...
var doBFS = function(graph, source) {
    ...
    while (!queue.isEmpty()) {
        ...
        for (var i = 0; i < graph[previous].length; i++) {
            var current = graph[previous][i];
            
            if (bfsInfo[current].distance === null) {
                bfsInfo[current].distance = bfsInfo[previous].distance+1;
                bfsInfo[current].predecessor = previous;
